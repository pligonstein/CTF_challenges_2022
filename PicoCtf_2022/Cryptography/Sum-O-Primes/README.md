We have so much faith in RSA we give you not just the product of the primes, but their sum as well!

[gen.py](https://artifacts.picoctf.net/c/184/gen.py)

[output.txt](https://artifacts.picoctf.net/c/184/output.txt)

For this challenge we're provided with two files. The first one it the code that was used to encrypt the flag and the second one is the output:

`x = `

`n = `

`c = `

After analyzing the gen.py code we see that p and q are used both used to calculate x as well as n value

`x = p + q`

`n = p * q`

So we can actually do some math to calculate p and q values so we can decode that cipher and get the flag. 

`p = (x + isqrt(x*x-4*n))//2`

`q = x - p`

So after we find out those values it's really easy for us to make a script and find out the flag.

`0x7069636f4354467b31323661393461627d`

After decoding this from hex to string we get the flag.

`picoCTF{126a94ab}`


